<script lang="ts">
	...
	let pages = $derived.by(() => {
		const cur = 1;
		const total = Math.ceil(productsMock.all.length / pageSize);
	...
	<div class="flex items-start gap-4">
		...
		<section>
			<div class="my-2 flex items-end justify-between">
				<p class="text-primary">Mostrando 1-{pageSize} de {productsMock.all.length} resultados</p>
			...
			<div class="grid-sp grid grid-cols-4 gap-6">
				{#each productsMock.all.slice(0, pageSize) as product}
					<ProductCard {product} />
				{/each}
			</div>
			<div>
